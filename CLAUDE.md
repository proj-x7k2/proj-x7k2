...
2. 작업 전에 `obsidian_notes/overview.md`를 읽어 현재 주제 목록을 파악하고, 필요하면
   ...
   **이미 페이지가 있는 논문은 새로 만들지 말고**, 빠진 단계(메타데이터의 `topics`,
   주제 페이지의 근거 통합과 "관련 논문" 링크)만 마무리합니다.
   스크립트는 "논문 페이지가 있고, `topics`에 적힌 모든 주제 페이지가 그 논문 파일을
   링크하고 있어야" 처리 완료로 인정합니다.
   작업은 하위 에이전트나 백그라운드 작업에 위임하지 말고 직접 순서대로 끝까지 수행합니다.
