...
#   website/template.html (틀)과 data/papers.json + data/topics.json (내용)을
#   합쳐서, website/index.html (실제로 여는 파일)을 새로 만듭니다.
#
...
def build():
    papers = _load_json(PAPERS_JSON_PATH)
    topics = build_topics()  # data/topics.json도 이 안에서 함께 생성됩니다.

    with open(TEMPLATE_PATH, "r", encoding="utf-8") as f:
        template = f.read()

    replacements = {
        "/*__PAPERS_JSON__*/ []": papers,
        "/*__TOPICS_JSON__*/ []": topics,
    }
    ...
    print(f"website/index.html 생성 완료 (논문 {len(papers)}편, 주제 {len(topics)}개 포함)")
